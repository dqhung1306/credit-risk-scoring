...
def load_assets():
    ...
    if not api_is_available():
        ...
        # Thử nhiều đường dẫn: local (../models) và Streamlit Cloud (/mount/src/.../models)
        candidate_paths = [
            os.path.join(current_dir, '..', 'models', 'lgbm_best_model.pkl'),
            os.path.join(current_dir, 'models', 'lgbm_best_model.pkl'),
            os.path.join(current_dir, '..', '..', 'models', 'lgbm_best_model.pkl'),
        ]
        ...
        if model_path is None:
            st.error(
                "❌ Không tìm thấy model local. "
                "Trên Streamlit Cloud cần chạy kèm API server hoặc đặt model vào thư mục 'models/' "
                "ngay trong repo (cùng cấp với file main.py)."
            )
        ...
        try:
            ...
            st.info(f"✅ Đã load model từ: `{model_path}`")
        except Exception as e:
